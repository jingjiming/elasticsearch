...
public class VCodeUtil {
    ...
    public static char[] generateVCode() {
        ...
        for (int i = 0; i < LENGTH; i++) {
            int rand = (int) (secureRandom.nextDouble() * chars.length());
    ...
    private static Graphics drawBackground(Graphics graphics) {
        ...
        // 随机产生120个干扰点
        for (int i = 0; i < 120; i++) {
            int x = (int) (secureRandom.nextDouble() * WIDTH);
            int y = (int) (secureRandom.nextDouble() * HEIGHT);

            int red = (int) (secureRandom.nextDouble() * 255);
            int green = (int) (secureRandom.nextDouble() * 255);
            int blue = (int) (secureRandom.nextDouble() * 255);
